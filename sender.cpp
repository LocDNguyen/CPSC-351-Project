...
unsigned long sendFile(const char* fileName)
{
	...
	while(!feof(fp))
	{
		...
		/* Send a message to the receiver telling him that the data is ready
 		 ...
 		 */
		sent = msgsnd(msqid, &sndMsg, (sizeof(message) - sizeof(long)), 0);
		if (sent == -1)
		{
			perror("msgsnd");
			exit(1);
		...
		/* Wait until the receiver sends us a message of type RECV_DONE_TYPE telling us
 		 ...
 		 */
		receive = msgrcv(msqid, &rcvMsg, (sizeof(ackMessage) - sizeof(long)), RECV_DONE_TYPE, 0);
 		if (receive == -1)
 		{
 			perror("msgrcv");
 			exit(1);
...
void sendFileName(const char* fileName)
{
	...
	sent = msgsnd(msqid, &fileMsg, (sizeof(fileNameMsg) - sizeof(char)), 0);
	if (sent == -1)
	{
		perror("msgsnd");
		exit(1);
