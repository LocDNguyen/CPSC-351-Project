...
unsigned long sendFile(const char* fileName)
{
	...
	while(!feof(fp))
	{
		...
		/* DONE TODO: Send a message to the receiver telling him that the data is ready
		...
		/* DONE TODO: Wait until the receiver sends us a message of type RECV_DONE_TYPE telling us
 		 * that he finished saving a chunk of memory.
 		 */
		/*cout << "before receive message" << endl;
		...
		cout << "after receive message" << endl;*/
	}


	/** DONE TODO: once we are out of the above loop, we have finished sending the file.
...
void sendFileName(const char* fileName)
{
	...
	/* DONE TODO: Make sure the file name does not exceed
	...
	/* DONE TODO: Create an instance of the struct representing the message
	 * containing the name of the file.
	 */
	fileNameMsg fileMsg;

	/* TODO: Set the message type FILE_NAME_TRANSFER_TYPE */
	fileMsg.mtype = FILE_NAME_TRANSFER_TYPE;

	/* TODO: Set the file name in the message */
	strncpy(fileMsg.fileName, fileName, fileNameSize + 1);

	/* DONE TODO: Send the message using msgsnd */
	sent = msgsnd(msqid, &fileMsg, (sizeof(fileNameMsg) - sizeof(char)), 0); // Why isn't "fileNameMsg" glowing mint?
	if (sent == -1)
	{
		perror("msgsnd");
		exit(1);
